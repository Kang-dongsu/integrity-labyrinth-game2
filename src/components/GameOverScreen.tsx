...
const GameOverScreen: React.FC<GameOverScreenProps> = ({ gameState, onRestart }) => {
  ...
  useEffect(() => {
    ...
    // For debugging - show Firebase connection status
    console.log('Firebase connection status:', {
      isConnected,
      usingMock: !isConnected,
  ...
  return (
    <div className="text-center bg-slate-800/50 p-8 rounded-lg shadow-2xl backdrop-blur-sm animate-fade-in">
      ...
      {showDebugInfo && (
        <div className="mt-4 p-4 bg-slate-900/50 rounded-lg text-left text-sm text-gray-400">
          <p className="font-bold mb-2">디버그 정보:</p>
          <p>Firebase 연결 상태: {firebaseConnected ? '연결됨' : '연결되지 않음'}</p>
          <p>Firebase 사용 중: {firebaseConnected ? '예' : '아니오 (Mock 데이터 사용)'} </p>
          <p>Firebase API 키 설정: {import.meta.env.VITE_FIREBASE_API_KEY ? '됨' : '안됨'}</p>
          {firebaseConnected && <p>기록이 Firebase와 로컬에 모두 저장됩니다.</p>}
          {!firebaseConnected && <p>기록이 임시로만 저장됩니다.</p>}
        </div>
      )}
